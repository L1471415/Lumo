...
class LumoChatManager:
        ...
        match command_name:
            case "get_time":
                if len(command_args) == 0:
                    return {
                        "response": json.dumps(new_assistant_functions.get_time(location=None)),
                        "continue_response": True
                    }

                if len(command_args) == 1:
                    return {
                        "response": json.dumps(new_assistant_functions.get_time(location=command_args[0])),
                        "continue_response": True
                    }
            ...
            case "get_weather":
                if len(command_args) == 0:
                    return {
                        "response": json.dumps(new_assistant_functions.get_weather(location=None)),
                        "continue_response": True
                    }

                elif len(command_args) == 1:
                    return {
                        "response": json.dumps(new_assistant_functions.get_weather(location=command_args[0])),
                        "continue_response": True
                    }
            ...
            case "search_wikipedia":
                if len(command_args) == 0:
                    return {
                        "response": json.dumps(new_assistant_functions.search_wikipedia(query=command_args[0])),
                        "continue_response": True
                    }
    ...
    def _get_ollama_response(self, messages:list, model:str):
        '''Function to prompt an ollama model
